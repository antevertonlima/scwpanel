...
function init_painel() {
    ...
    set_tema('headerinc', load_css(array('foundation'), 'assets/scwpanel/foundation/stylesheets'), FALSE);
	set_tema('headerinc', load_css(array('style'), 'assets/scwpanel/css'), FALSE);
    set_tema('footerinc', load_js(array('jquery.min', 's_scripts'), 'assets/scwpanel/js'), FALSE);
	set_tema('footerinc', load_js(array('jquery.validate.min'), 'assets/scwpanel/lib/validate'), FALSE);
...
function init_site() {
    ...
    //carregamento dos models
    //setando o tema
    set_tema('titulo_padrao', 'Passeio Em Miami');
    set_tema('rodape', '<p>&copy; ' . date('Y') . ' | Todos os direitos reservados para <a href="http://passeioemmiami.com">PasseioEmMiami.com</a>
    '.anchor(get_setting('facebook'),' ', array('class' => 'table-actions icon-fb', 'title' => 'Facebook', 'target' => '_blank')).anchor('https://twitter.com/'.get_setting('twitter'),' ', array('class' => 'table-actions icon-tw', 'title' => 'Twitter', 'target' => '_blank')).' ');
    set_tema('template', 'site_view');

    set_tema('headerinc', load_css(array('normalize', 'foundation.min'), 'assets/css'), FALSE);
    set_tema('headerinc', load_css('foundation-icons', 'assets/css/foundation-icons'), FALSE);
    set_tema('headerinc', load_css('app', 'assets/css'), FALSE);
    set_tema('headerinc', load_js('modernizr', 'assets/js/vendor'), FALSE);
    set_tema('footerinc', load_js('jquery', 'assets/js/vendor'), FALSE);
    set_tema('footerinc', load_js('foundation.min', 'assets/js'), FALSE);
}
...
function load_css($arquivo = NULL, $pasta = 'assets/admin/css', $media = 'all') {
    if ($arquivo != NULL):
        $CI = & get_instance();
        $CI->load->helper('url');
        $retorno = '';
...
function load_js($arquivo = NULL, $pasta = 'assets/admin/js', $remoto = FALSE) {
    if ($arquivo != NULL):
        $CI = & get_instance();
        $CI->load->helper('url');
        $retorno = '';
...
function set_msg2($id = 'msgerro', $msg = NULL, $tipo = 'erro') {
    $CI = & get_instance();
    switch ($tipo):
        case 'erro':
            $CI->session->set_flashdata($id, '<div data-alert class="alert-box alert"><p>' . $msg . '</p><a href="#" class="close">&times;</a></div>');
            break;
        case 'sucesso':
            $CI->session->set_flashdata($id, '<div data-alert class="alert-box success"><p>' . $msg . '</p><a href="#" class="close">&times;</a></div>');
            break;
        default:
            $CI->session->set_flashdata($id, '<div data-alert class="alert-box"><p>' . $msg . '</p><a href="#" class="close">&times;</a></div>');
            break;
    endswitch;
}
...
function thumb($caminho, $imagem = NULL, $largura = 100, $altura = 75, $ratio = TRUE, $alt = NULL, $geratag = TRUE) {
    $CI = & get_instance();
    $CI->load->helper('file');
    ...
    if ($geratag && $retorno != FALSE)
        $retorno = '<img src="' . $retorno . '" alt="' . $alt . '" />';
    return $retorno;
}
function thumb_zurb($caminho, $imagem = NULL, $largura = 100, $altura = 75, $ratio = TRUE, $cap = NULL, $alt = NULL, $geratag = TRUE) {
    $CI = & get_instance();
    $CI->load->helper('file');
    $thumb = $largura . 'x' . $altura . '_' . $imagem;
    $thumbinfo = get_file_info('assets/uploads/thumbs'. $thumb);
    ...
    if ($geratag && $retorno != FALSE)
        $retorno = '<img src="' . $retorno . '" class="th radius" data-caption="' . $cap . '" alt="' . $alt . '" />';
    return $retorno;
}
